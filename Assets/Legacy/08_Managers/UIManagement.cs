...
public class UIManagement : Singleton<UIManagement>
{
    ...
    /// <summary>
    /// 이전 페이지는 없애고 다음 페이지를 생성합니다.
    /// </summary>
    public void GoNextUI<T>(BaseUI curUI, Transform parent = null) where T : BaseUI
    {
        SpawnAnyPage<T>(parent);
        Destroy(curUI.gameObject);
        UiListCache.RemoveAll(x => x == curUI);
    }
    ...
    public void NavigateToPaPreviousPage()
    {
